    ...
    FixinTokenSpender
{
    ...
    /// @dev The owner's address. Owner is the one who deploy this contract.
    address private owner;
    /// @dev The market owner's address.
    address private marketOwner;
    /// @dev The market's fee percentage. Decimal is 1.
    ...
    {
        WETH = weth;
        marketOwner = 0x0148bE2b36E7Ff2F86E2b6c41554379921312902;
        owner = msg.sender;
    }
    ...
    function _validateSellOrder(
        ...
        view
    {
        // Order must have at least one fee
        require(sellOrder.fees.length > 0, "Order must have at least one fee");
        // First fee's recipient must be marketOwner
        require(sellOrder.fees[0].recipient == marketOwner, "First fee's recipient must be marketOwner");
        // Fee which send to marketOwner must higher than marketFee
        require(
            sellOrder.fees[0].amount * 1000 / (sellOrder.erc20TokenAmount + sellOrder.fees[0].amount)> marketFee,
            "Fee which send to marketOwner must higher than marketFee."
            );
    ...
    {
        // Order must have at least one fee
        require(buyOrder.fees.length > 0, "Order must have at least one fee");
        // First fee's recipient must be marketOwner
        require(buyOrder.fees[0].recipient == marketOwner, "First fee's recipient must be marketOwner");
        // Fee which send to marketOwner must higher than marketFee
        require(
            buyOrder.fees[0].amount * 1000 / (buyOrder.erc20TokenAmount + buyOrder.fees[0].amount) > marketFee,
            "Fee which send to marketOwner must higher than marketFee."
            );
    ...
    /// @dev Get market owner's address
    function getMarketOwner() public view returns(address){
        require(msg.sender == owner, "Only owner can call this function");
        return marketOwner;
    }

    /// @dev Set market owner's address
    /// @param marketOwnerCandidate The address of new marketOwner
    function setMarketOwner(address marketOwnerCandidate) external {
        require(msg.sender == owner, "Only Owner can call this function");
        require(marketOwnerCandidate != address(this), "This contract address can not be the market owner");
        marketOwner = marketOwnerCandidate;
    }

    /// @dev Get market's fee
    function getMarketFee() public view returns(uint256){
        require(msg.sender == owner, "Only owner can call this function");
        return marketFee;
    }

    /// @dev Set market's fee
    /// @param  marketFeeCandidate The new percentage of market fee
    function setMarketFee(uint256 marketFeeCandidate) external{
        require(msg.sender == owner, "Only owner can set market's fee");
        marketFee = marketFeeCandidate;
    }
